...
{
    public Task<TenantSubscription> AssignPlanAsync(Guid tenantId, Guid planId, string? notes, string? changedByUserId, CancellationToken ct = default)
        => Task.FromResult(TenantSubscription.Record(tenantId, planId, SubscriptionChangeReason.Initial, notes, changedByUserId, TimeProvider.System));
